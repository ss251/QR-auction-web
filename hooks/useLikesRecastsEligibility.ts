...
export function useLikesRecastsEligibility() {
  ...
  const checkEligibility = async (): Promise<EligibilityData> => {
    ...
    try {
      ...
      if (claimData && claimData.length > 0) {
        const likesOnlyClaim = claimData.find(
          (claim) => claim.option_type === 'likes'
        );
        const recastsOnlyClaim = claimData.find(
          (claim) => claim.option_type === 'recasts'
        );
        const bothClaim = claimData.find(
          (claim) => claim.option_type === 'both'
        );
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: frameContext?.user?.fid
      ? likesRecastsKeys.byFid(frameContext.user.fid)
      : ['likesRecasts-pending'],
  ...
  const recordClaimMutation = useMutation({
    mutationFn: async ({
      ...
      const amount =
        optionType === 'likes' ? 1000 : optionType === 'recasts' ? 1000 : 2000;

      const { error } = await supabase.from('likes_recasts_claims').insert({
    ...
    onSuccess: () => {
      ...
      if (frameContext?.user?.fid) {
        queryClient.invalidateQueries({
          queryKey: likesRecastsKeys.byFid(frameContext.user.fid),
        });
      }
    },
  });
